...
public sealed class SalesCreditNoteConfiguration : IEntityTypeConfiguration<SalesCreditNote>
{
    public void Configure(EntityTypeBuilder<SalesCreditNote> builder)
    {
        builder.ToTable("sales_credit_notes");
        ...
        {
            additions.ToTable("sales_credit_note_additions");
            additions.WithOwner().HasForeignKey("sales_credit_note_id");
        ...
        builder.HasMany(current => current.Items)
            .WithOne(item => item.SalesCreditNote)
            .HasForeignKey(item => item.SalesCreditNoteId)
