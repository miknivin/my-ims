...
internal static class PurchaseCreditNoteHandlers
{
    ...
    internal static async Task<IResult> CreateAsync(CreatePurchaseCreditNoteRequest request, AppDbContext dbContext, CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (nextStatus == PurchaseCreditNoteStatus.Submitted)
        {
            var settlementError = await ApplyInvoiceSettlementAsync(dbContext, purchaseCreditNote, cancellationToken);
            if (settlementError is not null)
            {
                return TypedResults.BadRequest(new ApiResponse<object>(false, settlementError, null));
            }
        }
        else if (purchaseCreditNote.Status == PurchaseCreditNoteStatus.Submitted && nextStatus == PurchaseCreditNoteStatus.Cancelled)
        {
            var settlementError = await ReverseInvoiceSettlementAsync(dbContext, purchaseCreditNote, cancellationToken);
            if (settlementError is not null)
            {
                return TypedResults.BadRequest(new ApiResponse<object>(false, settlementError, null));
            }
    ...
    private static async Task<string?> ApplyInvoiceSettlementAsync(
        AppDbContext dbContext,
        PurchaseCreditNote purchaseCreditNote,
        CancellationToken cancellationToken)
    {
        var purchaseInvoice = await LoadSettlementInvoiceAsync(dbContext, purchaseCreditNote, cancellationToken);
        if (purchaseInvoice.Error is not null)
        {
            return purchaseInvoice.Error;
        }

        var creditAmount = RoundAmount(purchaseCreditNote.Footer.NetTotal);
        if (creditAmount <= 0)
        {
            return null;
        }

        var currentBalance = RoundAmount(purchaseInvoice.Invoice!.FinancialDetails.Balance);
        if (creditAmount > currentBalance)
        {
            return $"Purchase credit note amount exceeds outstanding balance for purchase invoice {purchaseInvoice.Invoice.Document.No}.";
        }

        purchaseInvoice.Invoice.FinancialDetails.Balance = RoundAmount(currentBalance - creditAmount);
        purchaseInvoice.Invoice.UpdatedAtUtc = DateTime.UtcNow;

        return null;
    }

    private static async Task<string?> ReverseInvoiceSettlementAsync(
        AppDbContext dbContext,
        PurchaseCreditNote purchaseCreditNote,
        CancellationToken cancellationToken)
    {
        ...
        purchaseInvoice.Invoice!.FinancialDetails.Balance = RoundAmount(
            purchaseInvoice.Invoice.FinancialDetails.Balance + creditAmount);
        purchaseInvoice.Invoice.UpdatedAtUtc = DateTime.UtcNow;

        return null;
    }

    private static async Task<PurchaseCreditNoteSettlementInvoiceResult> LoadSettlementInvoiceAsync(
        AppDbContext dbContext,
        PurchaseCreditNote purchaseCreditNote,
        CancellationToken cancellationToken)
    {
        if (purchaseCreditNote.SourceRef.ReferenceId is null || purchaseCreditNote.SourceRef.ReferenceId == Guid.Empty)
        {
            return PurchaseCreditNoteSettlementInvoiceResult.Invalid("Source purchase invoice is required.");
        }

        var purchaseInvoice = await dbContext.PurchaseInvoices
            .FirstOrDefaultAsync(current => current.Id == purchaseCreditNote.SourceRef.ReferenceId.Value, cancellationToken);
        if (purchaseInvoice is null)
        {
            return PurchaseCreditNoteSettlementInvoiceResult.Invalid("Selected purchase invoice reference does not exist.");
        }

        if (purchaseInvoice.Status == PurchaseInvoiceStatus.Cancelled)
        {
            return PurchaseCreditNoteSettlementInvoiceResult.Invalid($"Purchase invoice {purchaseInvoice.Document.No} is cancelled.");
        }

        if (purchaseInvoice.VendorInformation.VendorId != purchaseCreditNote.VendorInformation.VendorId)
        {
            return PurchaseCreditNoteSettlementInvoiceResult.Invalid("Selected vendor does not match the referenced purchase invoice.");
        }

        return PurchaseCreditNoteSettlementInvoiceResult.Valid(purchaseInvoice);
    }
    ...
    private sealed record PurchaseCreditNoteSettlementInvoiceResult(string? Error, PurchaseInvoice? Invoice = null)
    {
        public static PurchaseCreditNoteSettlementInvoiceResult Valid(PurchaseInvoice invoice) => new(null, invoice);

        public static PurchaseCreditNoteSettlementInvoiceResult Invalid(string error) => new(error);
    }
