...
public sealed class PurchaseCreditNoteConfiguration : IEntityTypeConfiguration<PurchaseCreditNote>
{
    public void Configure(EntityTypeBuilder<PurchaseCreditNote> builder)
    {
        ...
        {
            additions.ToTable("purchase_credit_note_additions");
            additions.WithOwner().HasForeignKey("purchase_credit_note_id");
        ...
        builder.HasMany(current => current.Items)
            .WithOne(item => item.PurchaseCreditNote)
            .HasForeignKey(item => item.PurchaseCreditNoteId)
