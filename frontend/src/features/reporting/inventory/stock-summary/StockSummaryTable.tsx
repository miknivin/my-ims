...
import { StockSummaryEnhancedRow } from "@/redux/api/inventoryReportsApi";
...
export default function StockSummaryTable({ rows, groupBy, totalQuantity, isLoading, isError }: Props) {
  const byWarehouse = groupBy === "warehouse";
  // category mode: 4 cols (Code, Item, UOM, Qty)
  // warehouse mode: 5 cols (Code, Item, UOM, Category, Qty)
  const colSpan = byWarehouse ? 5 : 4;
  const dataColSpan = byWarehouse ? 4 : 3;
  ...
  const groups = useMemo(() => {
    const key = (r: StockSummaryEnhancedRow) =>
      byWarehouse
  ...
  if (isError) {
    return (
      <ReportLayout.EmptyState title="Failed to load" description="Unable to fetch stock summary. Please try again." />
    );
  }
    ...
    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
      <thead className="bg-gray-50 dark:bg-gray-800">
        <tr>
          ...
          {byWarehouse && (
            <th className="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Category</th>
          )}
          ...
          return (
            <Fragment key={group.name}>
              ...
              >
                {isCollapsed ? (
                  <>
                    <td colSpan={dataColSpan} className="px-3 py-2 text-xs font-bold uppercase tracking-widest text-gray-600 dark:text-gray-300">
                  ...
                  <td colSpan={colSpan} className="px-3 py-2 text-xs font-bold uppercase tracking-widest text-gray-600 dark:text-gray-300">
                    <span className="flex items-center gap-1.5">
                ...
                <>
                  ...
                  {group.rows.length > 1 && (
                    <tr className="border-t border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/30">
                      <td colSpan={dataColSpan} className="px-3 py-2 pl-5 text-xs font-semibold text-gray-600 dark:text-gray-300">
                        Total {group.name}
                      </td>
      ...
      <tfoot className="border-t-2 border-gray-300 bg-gray-100 dark:border-gray-600 dark:bg-gray-700/50">
        <tr>
          <td colSpan={dataColSpan} className="px-3 py-3 text-left text-sm font-bold uppercase text-gray-800 dark:text-white/90">
            Grand Total
          </td>
