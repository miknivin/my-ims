...
export default function InventoryValuationTable({ rows, totalQuantity, totalValue, isLoading, isError }: Props) {
  const groups = useMemo(() => {
    const names = [...new Set(rows.map((r) => r.categoryName ?? "Uncategorized"))];
  ...
  if (isError) {
    return (
      <ReportLayout.EmptyState
        ...
        description="Unable to fetch inventory valuation. Please try again."
      />
    );
  }
  ...
  if (rows.length === 0) {
    return (
      <ReportLayout.EmptyState
        ...
        description="No items match the selected filters as of this date."
      />
    );
  }
