...
import { useGetPayablesAgeingQuery } from "@/redux/api/receivablesPayablesApi";
...
export default function PayablesAgeingPage() {
  const [asOfDate, setAsOfDate] = useState(today);
  const { data, isLoading, isError } = useGetPayablesAgeingQuery({ asOfDate });

  return (
    <ReportLayout>
      <ReportLayout.Breadcrumb title="Payables Ageing" />
      ...
      {data && (
        <ReportLayout.Summary className="xl:grid-cols-5">
          ...
          <ReportLayout.Metric
            ...
            hint={data.totals.over90 > 0 ? "Critical — overdue to suppliers" : undefined}
          />
