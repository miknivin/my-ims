...
import { useGetSalesDebitNoteByIdQuery } from "@/redux/api/salesDebitNoteApi";
...
export default function SalesDebitNoteViewPage() {
  const { id } = useParams<{ id: string }>();
  const fmtDate = useFmtDate();
  const { data, isLoading, isError } = useGetSalesDebitNoteByIdQuery(id ?? "", { skip: !id });

  return (
    <ViewPageShell
      title="Sales Debit Note"
      backPath="/operations/adjustments/sales-debit-notes"
      isLoading={isLoading}
      isError={isError}
      isEmpty={!data}
    >
      {data && (
        <>
          <ViewSection title="Document">
            <ViewGrid>
              <ViewField label="SDN No" value={data.document.no} strong />
