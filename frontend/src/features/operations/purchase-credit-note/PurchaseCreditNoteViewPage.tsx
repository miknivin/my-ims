...
import { useGetPurchaseCreditNoteByIdQuery } from "@/redux/api/purchaseCreditNoteApi";
...
export default function PurchaseCreditNoteViewPage() {
  const { id } = useParams<{ id: string }>();
  const fmtDate = useFmtDate();
  const { data, isLoading, isError } = useGetPurchaseCreditNoteByIdQuery(id ?? "", { skip: !id });

  return (
    <ViewPageShell
      title="Purchase Credit Note"
      backPath="/operations/adjustments/purchase-credit-notes"
      isLoading={isLoading}
      isError={isError}
      isEmpty={!data}
    >
      {data && (
        <>
          <ViewSection title="Document">
            <ViewGrid>
              <ViewField label="PCN No" value={data.document.no} strong />
          ...
          {data.additions.length > 0 && (
            <ViewSection title="Additions / Deductions">
              <ViewTable>
                <thead className="bg-gray-50 dark:bg-gray-800">
                  <tr>
