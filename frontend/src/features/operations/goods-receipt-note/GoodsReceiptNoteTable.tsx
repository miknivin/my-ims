import { GoodsReceiptNoteListItem } from "@/redux/api/goodsReceiptNoteApi";
import {
  Table,
  TableBody,
  TableCell,
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {goodsReceipts.map((goodsReceipt) => (
                <TableRow key={goodsReceipt.id}>
                  <TableCell className="px-5 py-4 text-start text-theme-sm font-medium text-gray-800 dark:text-white/90">{goodsReceipt.no}</TableCell>
                  <TableCell className="px-4 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">{formatDate(goodsReceipt.date)}</TableCell>
                  <TableCell className="px-4 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">{goodsReceipt.vendorName}</TableCell>
                  <TableCell className="px-4 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">{goodsReceipt.netTotal.toFixed(2)}</TableCell>
                  ...
                  <TableCell className="px-4 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">{formatDate(goodsReceipt.createdAtUtc, true)}</TableCell>
                  <TableCell className="px-4 py-3 text-start text-theme-sm text-gray-500 dark:text-gray-400">{formatDate(goodsReceipt.updatedAtUtc, true)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
