...
import { useGetPurchaseDebitNoteByIdQuery } from "@/redux/api/purchaseDebitNoteApi";
...
export default function PurchaseDebitNoteViewPage() {
  const { id } = useParams<{ id: string }>();
  const fmtDate = useFmtDate();
  const { data, isLoading, isError } = useGetPurchaseDebitNoteByIdQuery(id ?? "", { skip: !id });

  return (
    <ViewPageShell
      title="Purchase Debit Note"
      backPath="/operations/adjustments/purchase-debit-notes"
      isLoading={isLoading}
      isError={isError}
      isEmpty={!data}
    >
      {data && (
        <>
          <ViewSection title="Document">
            <ViewGrid>
              <ViewField label="PDN No" value={data.document.no} strong />
