...
import { useGetSalesInvoiceByIdQuery, useDownloadSalesInvoicePdfMutation } from "@/redux/api/salesInvoiceApi";
...
export default function SalesInvoiceViewPage() {
  ...
  const { data, isLoading, isError } = useGetSalesInvoiceByIdQuery(id ?? "", { skip: !id });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `SI-${data.document.no}.pdf`;
      a.click();
  ...
  return (
    <ViewPageShell
      title="Sales Invoice"
      backPath="/operations/sales-invoice"
      isLoading={isLoading}
      isError={isError}
      isEmpty={!data}
      actions={
      ...
      {data && (
        <>
          <ViewSection title="Document">
            <ViewGrid>
              <ViewField label="SI No" value={data.document.no} strong />
              <ViewField label="Date" value={fmtDate(data.document.date)} />
              <ViewField label="Due Date" value={fmtDate(data.document.dueDate)} />
              <ViewField label="Salesperson" value={data.document.salespersonName} />
            </ViewGrid>
          ...
          <ViewSection title="Customer">
            <ViewGrid>
              ...
              <ViewField label="GSTIN" value={data.customerInformation.customerGstinSnapshot} />
