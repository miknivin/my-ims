...
export default function FinancialDetailsSection() {
  const { state, setFinancialDetails, editConfig } = usePurchaseOrderEditForm();
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <select
            ...
            onChange={(event) => setFinancialDetails({ paymentMode: event.target.value })}
          >
        ...
        <div>
          ...
          {isSubmitted ? (
            <input className={disabledClass} value={state.financialDetails.currencyLabel} disabled />
          ) : (
            <AutocompleteSelect
