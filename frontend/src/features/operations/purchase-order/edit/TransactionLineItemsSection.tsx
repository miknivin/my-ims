...
import { loadColumnPrefs, saveColumnPrefs } from "@/features/operations/shared/columnPreferences";

const ACTION_COLUMN_WIDTH = 90;
const INDEX_COLUMN_WIDTH = 55;

interface EditTransactionLineItemsSectionProps<
  TLine,
  TContext,
...
function EditTransactionLineItemRow<TLine, TContext, TKey extends string = string>({
  displayIndex,
  line,
  columns,
  templateColumns,
...
export default function EditTransactionLineItemsSection<
  TLine,
  TContext,
  TKey extends string = string,
>({
  ...
  storageKey,
  readOnly = false,
}: EditTransactionLineItemsSectionProps<TLine, TContext, TKey>) {
  ...
  const gridTemplateColumns = useMemo(() => {
    const parts = [
      ...
      ...(readOnly ? [] : [`${ACTION_COLUMN_WIDTH}px`]),
    ];
    return parts.join(" ");
  }, [columnWidths, readOnly, visibleColumns]);

  const startResize = (key: TKey, startX: number, startWidth: number, minWidth: number) => {
            ...
            {visibleColumns.map((column) => (
              ...
              >
                ...
                >
                  ...
                  <span className="shrink-0">{getSortIndicator(column.key)}</span>
                </button>
                <button
                  type="button"
                  aria-label={`Resize ${column.label}`}
                  onMouseDown={(event) =>
                    startResize(column.key, event.clientX, columnWidths[column.key], column.minWidth)
                  }
