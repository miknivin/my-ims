...
export default function ProcurementSection() {
  const { state, setProductInformation } = usePurchaseOrderForm();

  return (
    <TransactionSectionCard title="Product / Reference Details">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <input
            ...
            onChange={(event) =>
              setProductInformation({ reference: event.target.value })
            }
        ...
        <div>
          ...
          <input
            ...
            onChange={(event) =>
              setProductInformation({ mrNo: event.target.value })
            }
            placeholder="Material request reference"
          />
        </div>
