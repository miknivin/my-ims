...
import { SalesAdjustmentNoteNature } from "../types/types";
...
function formatNatureLabel(value: SalesAdjustmentNoteNature) {
  switch (value) {
    case "RateDifference":
...
export default function OrderDetailsSection() {
  const {
    ...
    hydrateFromSalesInvoice,
  } = useSalesAdjustmentNoteForm();
  const { data: salesInvoices = [] } = useGetSalesInvoicesQuery();
  const [getSalesInvoiceById] = useLazyGetSalesInvoiceByIdQuery();
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
          ...
          <select
            ...
            onChange={(event) =>
              setNoteNature(event.target.value as SalesAdjustmentNoteNature)
            }
          >
        ...
        <div className="md:col-span-2">
          <label className={labelClass}>Source Sales Invoice</label>
          <AutocompleteSelect
            ...
            search={async (keyword) => {
              ...
              return salesInvoices
                .filter((salesInvoice) =>
                  [salesInvoice.no, salesInvoice.customerName].some((value) =>
            ...
            onSelect={async (item) => {
              ...
              try {
                const salesInvoice = await getSalesInvoiceById(item.id).unwrap();
