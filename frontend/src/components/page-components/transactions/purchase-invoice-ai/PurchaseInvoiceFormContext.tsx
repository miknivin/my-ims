...
export function PurchaseInvoiceFormProvider({
  ...
  initialState?: Partial<PurchaseInvoiceFormState> | null;
}) {
  const [state, setState] = useState<PurchaseInvoiceFormState>(() => {
    if (initialState) {
      return createPurchaseInvoiceFormState(initialState);
    }

    return loadPurchaseInvoiceAiDraft() ?? createPurchaseInvoiceFormState();
  });

  useEffect(() => {
    savePurchaseInvoiceAiDraft(state);
  }, [state]);
